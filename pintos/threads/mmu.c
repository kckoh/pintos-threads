...
static bool pdp_for_each(uint64_t *pdp, pte_for_each_func *func, void *aux, unsigned pml4_index)
{
    for (unsigned i = 0; i < PGSIZE / sizeof(uint64_t *); i++)
    {
        ...
        if (((uint64_t)pde) & PTE_P)
            if (!pgdir_for_each((uint64_t *)PTE_ADDR(pde), func, aux, pml4_index, i)) return false;
    }
    return true;
}
 ...
 * UADDR is unmapped. */
/* uaddr가 담긴 물리 페이지의 커널 접근 주소를 반환*/
 ...
 * failed. */
/* PML4(Page Map Level 4)에서 사용자 가상 페이지 UPAGE를
 * 커널 가상 주소 KPAGE로 식별되는 물리 프레임에 매핑합니다.
 * UPAGE는 이미 매핑되어 있지 않아야 합니다. KPAGE는 아마도
 * palloc_get_page()를 통해 사용자 풀에서 얻은 페이지여야 합니다.
...
{
    ASSERT(pg_ofs(upage) == 0);    // UPAGE가 페이지 단위로 정렬되어 있는지 확인
    ASSERT(pg_ofs(kpage) == 0);    // KPAGE가 페이지 단위로 정렬되어 있는지 확인
    ASSERT(is_user_vaddr(upage));  // UPAGE가 사용자 영역 주소인지 확인
    ASSERT(pml4 != base_pml4);     // PML4가 커널 기본 PML4가 아닌지 확인

    // UPAGE에 해당하는 PTE(Page Table Entry)를 찾습니다. 없으면 생성합니다 (create=1).
    uint64_t *pte = pml4e_walk(pml4, (uint64_t)upage, 1);

    if (pte)
        // PTE를 설정합니다:
        // vtop(kpage): KPAGE(커널 가상 주소)에 해당하는 물리 주소
        // PTE_P: Present 비트 (페이지가 메모리에 존재함)
        // rw ? PTE_W : 0: 쓰기 가능 여부에 따라 Writable 비트 설정
        // PTE_U: User 비트 (사용자가 접근 가능함)

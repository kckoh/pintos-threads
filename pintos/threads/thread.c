...
bool cond_priority_less(struct list_elem *a, struct list_elem *b,
  ...
  struct thread *athread =
      list_entry(list_front(&asema->semaphore.waiters), struct thread, elem);
  struct thread *bthread =
      list_entry(list_front(&besma->semaphore.waiters), struct thread, elem);
...
void check_donate_priority(void) {
  ...
  if (!list_empty(&curr->donaters)) {
    int donate_max_priority =
        list_entry(list_max(&curr->donaters, donate_less, NULL), struct thread,
                   donate_elem)
            ->priority;

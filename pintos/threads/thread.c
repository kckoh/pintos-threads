...
void thread_yield(void)
{
    ...
    old_level = intr_disable();
    if (curr != idle_thread) list_insert_ordered(&ready_list, &curr->elem, priority_more, NULL);
...
void check_donate_priority(void)
{
    ...
    {
        int donate_max_priority =
            list_entry(list_max(&curr->donaters, donate_less, NULL), struct thread, donate_elem)
                ->priority;
        if (donate_max_priority > curr->original_priority) curr->priority = donate_max_priority;
    }
}

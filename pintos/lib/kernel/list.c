...
/* 보조 데이터 AUX가 주어졌을 때 LESS에 따라 LIST를 정렬 */
...
/* 보조 데이터 AUX를 고려하여 LIST의 적절한 위치에 ELEM을 삽입.
   LIST는 주어진 보조 데이터 AUX에 따라 정렬.
   LIST의 요소 개수에 따라 평균 O(n)의 시간이 소요*/
						 ...
						 list_less_func *less, void *aux)
{
	...
	for (e = list_begin(list); e != list_end(list); e = list_next(e))
		if (less(elem, e, aux)) // elem이 e보다 작으면 true
			break;
	return list_insert(e, elem); // e앞에 elem 삽입,
}

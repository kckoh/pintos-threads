 ...
 * 이 데이터 구조는 프로젝트 3의 Pintos Tour에 자세히 설명되어 있습니다.
 *
 * This is a standard hash table with chaining.  To locate an
 * element in the table, we compute a hash function over the
 * element's data and use that as an index into an array of
 * doubly linked lists, then linearly search the list.
 * 이것은 체이닝을 사용하는 표준 해시 테이블입니다. 테이블에서 요소를 찾으려면
 * 요소의 데이터에 대해 해시 함수를 계산하고 그 결과를 이중 연결 리스트의
 * 배열에 대한 인덱스로 사용한 다음, 리스트를 선형 검색합니다.
 *
 ...
 * 각 구조체는 struct hash_elem 멤버를 포함해야 합니다. 모든 해시 함수는
 * 이 `struct hash_elem`에 대해 작동합니다. hash_entry 매크로는
 * struct hash_elem을 다시 그것을 포함하는 구조체 객체로 변환할 수 있게 해줍니다.
 * 이는 연결 리스트 구현에서 사용된 것과 동일한 기법입니다. 자세한 설명은
 ...
 * MEMBER를 제공하십시오. 예제는 파일 상단의 큰 주석을 참조하십시오. */

...
static bool handle_syscall_fault(struct intr_frame* f)
{
    if (f->cs != SEL_KCSEG) return false;

    if (f->R.rax == 0) return false;
    // printf("done_주소임 : %d \n", f->R.rax);
    f->rip = f->R.rax;
    f->R.rax = -1;
    return true;
...
static void page_fault(struct intr_frame* f)
{
    ...
    if (fault_addr < KERN_BASE && handle_syscall_fault(f)) return;

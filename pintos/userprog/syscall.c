...
static int64_t get_user(const uint8_t* uaddr)
{
    ...
    if (uaddr == NULL || !is_user_vaddr(uaddr)) return -1;
...
void syscall_handler(struct intr_frame* f)
{
    ...
    switch (syscall_num)
    {
        ...
        case SYS_EXEC:
            if (sys_exec(f->R.rdi) < 0) sys_exit(-1);
            break;
...
static void valid_get_addr(void* addr)
{
    if (get_user(addr) < 0) sys_exit(-1);
}
/* 버퍼에서 가져오기 검사 */
static void valid_get_buffer(char* buffer, unsigned length)
{
    char* end = buffer + length - 1;
    if (get_user(buffer) < 0 || get_user(end) < 0) sys_exit(-1);
}
/* 버퍼에 쓰기 검사 */
static void valid_put_buffer(char* buffer, unsigned length)
{
    char* end = buffer + length - 1;
    if (put_user(buffer, 0) == 0 || put_user(end, 0) == 0) sys_exit(-1);
}
...
static void sys_close(int fd)
{
    ...
    if (file == NULL || file == (struct file*)STDIN_FILENO || file == (struct file*)STDOUT_FILENO)
    {
        fd_table[fd] = NULL;
        return;
    }
...
static int sys_read(int fd, void* buffer, unsigned length)
{
    ...
    if (fd < 0 || fd == STDOUT_FILENO || fd > thread_current()->fd_capacity) return -1;
...
static int sys_write(int fd, void* buffer, unsigned length)
{
    ...
    if (fd < 0 || fd == STDIN_FILENO || fd > thread_current()->fd_capacity) return -1;

    struct file* file = thread_current()->fd_table[fd];

    if (file == (struct file*)STDOUT_FILENO && thread_current()->fd_table[fd] != NULL)
    {
        /* stdout으로 write*/
...
static void sys_seek(int fd, unsigned position)
{
    if (fd < 2 || fd > thread_current()->fd_capacity) return;
    // sys_exit(-1);
...
static unsigned sys_tell(int fd)
{
    if (fd < 2 || fd > thread_current()->fd_capacity) return -1;
...
{
    struct thread* curr = thread_current();
    if (oldfd < 0 || newfd < 0 || oldfd > 10000 || newfd > 10000)
    {
        return -1;
    }
    // invalid oldfd; returns -1 and newfd is not closed
    if (curr->fd_table[oldfd] == NULL)
    {
        return -1;
    }

    // valid oldfd; newfd = oldfd -> does nothing and returns newfd
    if (oldfd == newfd)
    {
        return newfd;
    }

    // Expand fd_table if necessary
    if (curr->fd_capacity <= newfd)
    {
        int old_capacity = curr->fd_capacity;
        int new_capacity = newfd + 1;

        struct file** new_table = realloc(curr->fd_table, sizeof(struct file*) * new_capacity);
        if (new_table == NULL)
        {
            return -1;
        }
        ...
        // Initialize newly allocated entries to NULL
        for (int i = old_capacity; i < new_capacity; i++)
        {
            curr->fd_table[i] = NULL;
        }

        curr->fd_capacity = new_capacity;
    }

    // close if newfd was already opened
    ...
    lock_acquire(&file_lock);
    // copies from oldfd to newfd
    struct file* old_file = curr->fd_table[oldfd];

    if (old_file == (struct file*)STDIN_FILENO)
    {
        curr->fd_table[newfd] = (struct file*)STDIN_FILENO;
    }
    else if (old_file == (struct file*)STDOUT_FILENO)
    {
